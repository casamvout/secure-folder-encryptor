...
╔══════════════════════════════════════════════════════════════════════════════╗
║                         cryptolibo.py  v4.0                                ║
║                  Профессиональная библиотека шифрования                     ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Быстрый старт:                                                             ║
║    import cryptolibo                                                        ║
║                                                                             ║
...
║    enc = cryptolibo.encrypt.aes_gcm(key, "секрет")   # рекомендуется      ║
║    dec = cryptolibo.decrypt.aes_gcm(key, enc)                              ║
║                                                                             ║
║    enc = cryptolibo.encrypt.aes256(key, "секрет")                         ║
║    dec = cryptolibo.decrypt.aes256(key, enc)                               ║
║                                                                             ║
║    enc = cryptolibo.encrypt.wordlock("пароль", "секрет")                   ║
║    dec = cryptolibo.decrypt.wordlock("пароль", enc)                        ║
║                                                                             ║
║    # Быстрое шифрование (ключ генерируется автоматически)                  ║
║    enc, key = cryptolibo.quick_encrypt("данные")                           ║
...
║                                                                             ║
║    # Потоковое шифрование файлов (любой размер)                            ║
║    cryptolibo.encrypt_stream("big.mp4", "big.mp4.enc", key)               ║
║    cryptolibo.decrypt_stream("big.mp4.enc", "big.mp4", key)               ║
║                                                                             ║
║    # Хеширование                                                            ║
║    h = cryptolibo.hash.sha256("данные")                                    ║
║    h = cryptolibo.hash.argon2("пароль")        # для паролей               ║
║                                                                             ║
║    # HMAC подпись                                                           ║
║    sig = cryptolibo.sign.hmac_sha256(key, "данные")                        ║
║    ok  = cryptolibo.sign.verify(key, "данные", sig)                        ║
╚══════════════════════════════════════════════════════════════════════════════╝

Изменения v4.0:
  - Исправлен баг в _base58_encode (неверная логика ведущих нулей)
  - Убрано дублирование импорта AESGCM
  - Кэш деривации ключей (_key_cache) — ускорение повторных вызовов
  - CryptoVault — зашифрованное JSON-хранилище секретов
  - encrypt_stream / decrypt_stream — потоковое шифрование (>4GB файлы)
  - batch_encrypt / batch_decrypt — пакетное шифрование списка строк
  - encrypt.aes_gcm_file / decrypt.aes_gcm_file — удобные методы на классах
  - KeyManager.rotate — ротация ключа с перешифрованием
  - Hash.tree — Merkle-хеш дерево для списка данных
  - Utils.zeroize — безопасное затирание строк/байтов
  - Utils.timing_safe_sleep — константное время ответа
  - Улучшенные сообщения об ошибках с кодами
  - Полная аннотация типов (TypeAlias, Final)
  - Поддержка контекстного менеджера для CryptoVault
  - Метод capabilities() возвращает версии зависимостей
"""
...
# Размер блока для потокового шифрования (64 КБ)
_STREAM_CHUNK: int = 65_536
# Метка версии формата потокового файла
...
class DecryptionError(CryptoError):
    """Ошибка дешифрования (неверный ключ / повреждённые данные)."""
    code = "DECRYPTION_FAILED"


class IntegrityError(CryptoError):
    """Нарушение целостности данных (подпись/MAC не прошёл проверку)."""
    code = "INTEGRITY_VIOLATION"


class MissingDependency(CryptoError):
    """Требуемая зависимость не установлена."""
    code = "MISSING_DEPENDENCY"

    def __init__(self, package: str):
        super().__init__(
            f"Установите зависимость: pip install {package}\n"
            f"  Или все зависимости: pip install cryptography pycryptodome argon2-cffi"
        )
        self.package = package


class KeyError_(CryptoError):
    """Ошибка работы с ключом (неверный формат, размер и т.д.)."""
    code = "KEY_ERROR"
...
# ─── Кэш деривации ключей ─────────────────────────────────────────────────────

class _KeyCache:
    """Потокобезопасный LRU-кэш для дорогих операций деривации ключей."""
    ...
    def set(self, key: tuple, value: bytes) -> None:
        with self._lock:
            ...
            # Кэшируем по хешу ключа, а не по самому ключу —
            # чтобы не держать секрет в открытом виде в памяти
...
def _to_str(data: bytes) -> Union[str, bytes]:
    """Декодирует bytes в str если валидный UTF-8, иначе возвращает bytes."""
    try:
...
def _b64dec(data: str) -> bytes:
    ...
    try:
        ...
        raise DecryptionError(f"Неверный Base64: {e}") from e
...
def _derive_key(key: DataLike, size: int, salt: bytes = b"") -> bytes:
    """
    Детерминированное производство ключа нужного размера.

    С солью  — PBKDF2-HMAC-SHA256 (100k итераций).
    Без соли — HKDF-SHA256 (expand-only, info=b"cryptolibo/v4").
               Используется только для внутренних нужд (не для паролей!).

    Кэш хранит (SHA3-256(key), size, salt) → не держим секрет открытым.
    """
    raw = _to_bytes(key)

    # Ключ кэша — хеш от секрета, чтобы не хранить его в памяти открытым
    ...
    if salt:
        # Для паролей/пользовательских ключей с солью
        result = hashlib.pbkdf2_hmac("sha256", raw, salt, 100_000, dklen=size)
    else:
        # HKDF expand-only: PRK = raw (уже считаем энтропийным),
        # выдаём нужное количество байт через HMAC-цепочку
...
def _resolve_key(key: DataLike) -> DataLike:
    """Если key — путь к .key/.pem/.txt — загружает содержимое."""
...
def _key_to_raw(key: DataLike, size: int) -> bytes:
    """
    Превращает любой ключ (hex-строка, bytes, строка-пароль) в bytes нужного размера.
    Вызывает KeyError_ при явно неверном размере ключа.
    """
    if size not in (8, 16, 24, 32, 48, 64):
        raise KeyError_(f"Неверный размер ключа: {size} байт")
...
# ─── WordLock v2 — улучшенный собственный алгоритм ───────────────────────────

def _wl_stream(seed: bytes, length: int) -> bytes:
    """Криптографически стойкий поток байт из seed через SHA3-цепочку."""
    out = bytearray()
    counter = 0
...
def _wl_sbox(key_bytes: bytes) -> bytes:
    """S-box (256 байт) из ключа через KSA (как в RC4 но с SHA3)."""
...
def _wl_shuffle(seed: bytes, n: int) -> list:
    """Fisher-Yates перестановка на основе stream из seed."""
...
def _wordlock_crypt(word: str, data: bytes, encrypt: bool) -> Union[str, bytes]:
    ...
    Слои защиты:
      - Случайная соль (16 байт) → семантическая безопасность
      - S-box подстановка (KSA из SHA3)
      - XOR с потоком (SHA3-цепочка)
      - Fisher-Yates перестановка байт
      - HMAC-SHA256 тег целостности (16 байт)
    """
    w = _to_bytes(word)
    MAC_LEN  = 16
    SALT_LEN = 16
    ...
    if encrypt:
        ...
        if len(data) < SALT_LEN + MAC_LEN:
            raise DecryptionError("Данные слишком короткие или повреждены")
        ...
        if not _hmac.compare_digest(mac_calc, mac_given):
            raise IntegrityError("Неверное кодовое слово или данные повреждены (HMAC)")
...
class KeyManager:
    """Генерация, сохранение, загрузка и ротация криптографических ключей."""
    ...
    def generate(
        ...
        """
        Генерирует криптостойкий симметричный ключ.

        Args:
            bits:       128 / 192 / 256 / 512.
            save_to:    Путь для сохранения (None = не сохранять).
            encoding:   "hex" | "base64".
            passphrase: Защитить файл ключа паролем.
        """
        if bits % 8 != 0:
            raise KeyError_("bits должно быть кратно 8")
        if bits < 128:
            raise KeyError_("Минимальный размер ключа — 128 бит")
    ...
    def generate_rsa(
        ...
        """Генерирует пару RSA ключей. Возвращает (private_pem, public_pem)."""
        _require(CRYPTO_AVAILABLE, "cryptography")
        if bits < 2048:
            raise KeyError_("RSA ключ должен быть не менее 2048 бит")
        priv = _rsa_gen(
    ...
    def generate_ec(
        ...
        """
        Генерирует пару ECDSA/ECDH ключей.
        ...
        if curve not in curves:
            raise KeyError_(f"Неизвестная кривая: {curve}. Доступны: {list(curves)}")
    ...
    def generate_ed25519(
        ...
        """Генерирует пару Ed25519 ключей для подписи."""
        _require(CRYPTO_AVAILABLE, "cryptography")
    ...
    def generate_x25519(self) -> Tuple[str, str]:
        """Генерирует пару X25519 ключей для ECDH обмена."""
        _require(CRYPTO_AVAILABLE, "cryptography")
    ...
    def save(self, key: str, path: str, passphrase: Optional[str] = None) -> None:
        """Сохраняет ключ в файл (опционально с парольной защитой AES-GCM)."""
        p = Path(path)
        ...
        if passphrase:
            ...
            if CRYPTO_AVAILABLE:
                ...
                # Fallback: XOR stream (нет AEAD — нет аутентификации)
    ...
    def load(self, path: str, passphrase: Optional[str] = None) -> str:
        ...
        try:
            ...
            if payload.get("protected"):
                if not passphrase:
                    raise KeyError_("Файл ключа защищён паролем — передайте passphrase")
                ...
                if payload.get("v", 1) == 2 and CRYPTO_AVAILABLE:
                    ...
                    try:
                        ...
                        raise DecryptionError("Неверный пароль для файла ключа")
                else:
    ...
    def load_or_generate(
        ...
        """Загружает ключ если есть, иначе генерирует новый."""
    ...
    def rotate(
        ...
        """
        Ротация ключа — опционально перешифровывает список данных.

        Args:
            old_key:   Текущий ключ.
            new_key:   Новый ключ (если None — генерируется).
            data_list: Список зашифрованных строк для перешифрования.
            algo:      Алгоритм ("aes_gcm" | "wordlock").
        Returns:
    ...
    def hkdf_expand(
        ...
        """HKDF расширение ключа (для деривации подключей)."""
        _require(CRYPTO_AVAILABLE, "cryptography")
        hkdf = HKDF(
    ...
    def info(self, key: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality":       "отличный" if ent > 7.5 else ("хороший" if ent > 6.5 else "слабый"),
            "preview":       raw[:4].hex() + "...",
        }
...
class Encrypt:
    ...
    def aes_gcm(
        ...
        Устойчив к подделке и модификации.
    ...
    def aes_ctr(self, key: KeyLike, data: DataLike) -> str:
        """AES-256-CTR (потоковый режим, без паддинга)."""
        _require(CRYPTO_AVAILABLE, "cryptography")
        k     = _key_to_raw(key, 32)
    ...
    # ── AES-SIV (детерминированное AEAD) ─────────────────────────────────────

    def aes_siv(self, key: KeyLike, data: DataLike, aad: Optional[bytes] = None) -> str:
        """
        AES-SIV — детерминированное AEAD шифрование.
        Устойчив к повторному использованию nonce, не даёт утечки при коллизиях.
        Требует 512-бит ключ (два AES-256).
        """
        _require(CRYPTO_AVAILABLE, "cryptography")
    ...
    def chacha20_poly1305(self, key: KeyLike, data: DataLike) -> str:
        """ChaCha20-Poly1305 — быстрое AEAD шифрование (альтернатива AES-GCM)."""
        _require(CRYPTO_AVAILABLE, "cryptography")
        k     = _key_to_raw(key, 32)
        nonce = secrets.token_bytes(12)
    ...
    def chacha20(self, key: KeyLike, data: DataLike) -> str:
        """ChaCha20 без аутентификации (используйте chacha20_poly1305 при возможности)."""
    ...
    def salsa20(self, key: KeyLike, data: DataLike) -> str:
        """Salsa20 — высокоскоростной потоковый шифр."""
        if PYCRYPTODOME_AVAILABLE:
    ...
    # ── DES / 3DES (совместимость) ────────────────────────────────────────────

    def des(self, key: KeyLike, data: DataLike) -> str:
        """DES-CBC (устаревший, только для совместимости)."""
        _require(PYCRYPTODOME_AVAILABLE, "pycryptodome")
        k  = _key_to_raw(key, 8)
    ...
    def triple_des(self, key: KeyLike, data: DataLike) -> str:
        """3DES-CBC (устаревший, только для совместимости)."""
        _require(PYCRYPTODOME_AVAILABLE, "pycryptodome")
    ...
    def rc2(self, key: KeyLike, data: DataLike) -> str:
        """RC2-CBC (только для совместимости)."""
        _require(PYCRYPTODOME_AVAILABLE, "pycryptodome")
        k  = _key_to_raw(key, 16)
        iv = secrets.token_bytes(8)
    ...
    def rc4(self, key: KeyLike, data: DataLike) -> str:
        """RC4 (не рекомендуется для новых проектов)."""
        _require(PYCRYPTODOME_AVAILABLE, "pycryptodome")
        k  = _key_to_raw(key, 16)
    ...
    def rsa_oaep_sha512(self, public_key_pem: str, data: DataLike) -> str:
        """RSA-OAEP-SHA512 шифрование (более стойкий вариант)."""
        _require(CRYPTO_AVAILABLE, "cryptography")
        from cryptography.hazmat.primitives.serialization import load_pem_public_key
        pub = load_pem_public_key(_to_bytes(public_key_pem), backend=default_backend())
        ct  = pub.encrypt(
    ...
    def ecies(self, public_key_pem: str, data: DataLike) -> str:
        ...
        Генерирует эфемерную пару EC, выполняет ECDH, шифрует AES-GCM.
        Совместим с ключами P-256/P-384/P-521/secp256k1.
        """
        _require(CRYPTO_AVAILABLE, "cryptography")
        from cryptography.hazmat.primitives.serialization import load_pem_public_key
        pub = load_pem_public_key(_to_bytes(public_key_pem), backend=default_backend())
    ...
    def xor(self, key: KeyLike, data: DataLike) -> str:
        ...
        Ключ нормализуется через _key_to_raw (поддерживает hex, bytes, строку).
        К каждому шифрованию добавляется случайный 16-байтный nonce,
        который подмешивается в поток через HKDF — разные шифртексты
        для одинаковых данных (семантическая безопасность).

        Формат: base64(nonce[16] + ciphertext)
        """
        k     = _key_to_raw(key, 32)
        nonce = secrets.token_bytes(16)
        # Деривируем поток из ключа + nonce, чтобы каждый вызов давал
        # уникальный поток даже при одинаковом ключе и данных
    ...
    def vigenere(self, key: KeyLike, data: DataLike) -> str:
        """Шифр Виженера (расширенный на весь байтовый диапазон 0–255)."""
        k   = _to_bytes(key)
        d   = _to_bytes(data)
    ...
    def rot47(self, data: DataLike) -> str:
        """ROT47 — сдвиг всех печатных ASCII символов."""
        s = _to_bytes(data).decode("utf-8", errors="replace")
        return "".join(
    ...
    def polybius(self, data: DataLike) -> str:
        """
        Квадрат Полибия — заменяет каждую букву парой цифр (строка, столбец).
        I и J считаются одной буквой.
        """
        square = [
            ['A','B','C','D','E'],
            ['F','G','H','I','K'],
    ...
    def beaufort(self, key: KeyLike, data: DataLike) -> str:
        """Шифр Бофорта (C = Key - Plain mod 26). Инволютивный."""
    ...
    def rail_fence(self, data: DataLike, rails: int = 3) -> str:
        """Шифр железнодорожной ограды (Rail Fence)."""
        s = _to_bytes(data).decode("utf-8", errors="replace")
    ...
    def wordlock(self, word: str, data: DataLike) -> str:
        """
        WordLock v2 — собственный алгоритм шифрования по кодовому слову.

        Слои защиты:
          1. S-box подстановка (KSA из SHA3)
          2. XOR с потоком (SHA3-цепочка)
          3. Fisher-Yates перестановка байт
          + Случайная соль (семантическая безопасность)
          + HMAC-SHA256 тег целостности
        """
...
class Decrypt:
    ...
    def aes_gcm(self, key: KeyLike, data: str, aad: Optional[bytes] = None) -> Union[str, bytes]:
        ...
        raw = _b64dec(data)
        if len(raw) < 12 + 16:
            raise DecryptionError("Данные слишком короткие для AES-GCM")
        nonce, ct = raw[:12], raw[12:]
        try:
            return self._decode(AESGCM(k).decrypt(nonce, ct, aad))
        except Exception:
            raise DecryptionError("Неверный ключ, AAD или повреждённые данные")

    def aes_gcm_128(self, key: KeyLike, data: str, aad: Optional[bytes] = None) -> Union[str, bytes]:
        """AES-128-GCM дешифрование."""
        _require(CRYPTO_AVAILABLE, "cryptography")
        k   = _key_to_raw(key, 16)
        raw = _b64dec(data)
        nonce, ct = raw[:12], raw[12:]
    ...
    def _aes_cbc(self, key: KeyLike, data: str, ksize: int) -> Union[str, bytes]:
        ...
        raw = _b64dec(data)
        if len(raw) < 16:
            raise DecryptionError("Данные слишком короткие для AES-CBC")
        iv, ct = raw[:16], raw[16:]
        try:
            ...
            raise DecryptionError("Неверный ключ или повреждённые данные")
    ...
    def xor(self, key: KeyLike, data: str) -> Union[str, bytes]:
        """Дешифрование XOR. Ожидает формат от Encrypt.xor."""
        k   = _key_to_raw(key, 32)
        raw = _b64dec(data)
        if len(raw) < 16:
            raise DecryptionError("Данные слишком короткие для XOR (нет nonce)")
        nonce, ct  = raw[:16], raw[16:]
        stream_key = _hmac.new(k, nonce, "sha256").digest()
    ...
    def hex(self, data: str) -> Union[str, bytes]:
        try:
            ...
            raise DecryptionError(f"Неверный hex: {e}") from e
    ...
    def binary(self, data: str) -> Union[str, bytes]:
        ...
        try:
            ...
            raise DecryptionError(f"Неверный binary: {e}") from e

    def wordlock(self, word: str, data: str) -> Union[str, bytes]:
        """
        WordLock v2 — дешифровка по кодовому слову.
        Вызывает IntegrityError при неверном слове.
        """
...
class Hash:
    ...
    def pbkdf2(
        ...
        """PBKDF2 для паролей. Returns: (hash_hex, salt_hex)"""
    ...
    def argon2(
        ...
        """
        Argon2 — рекомендуемый KDF для паролей.
    ...
    def file_multi(self, path: str) -> Dict[str, str]:
        """Хеши файла всеми основными алгоритмами за один проход."""
        hashes_map = {
            "md5":     hashlib.md5(),
            "sha1":    hashlib.sha1(),
    ...
    def tree(self, items: List[DataLike], algo: str = "sha256") -> str:
        """
        Merkle-хеш дерево для списка данных.
        Возвращает корневой хеш. Полезно для верификации наборов данных.
        """
        if not items:
        ...
        while len(leaves) > 1:
            if len(leaves) % 2 != 0:
                leaves.append(leaves[-1])  # дублируем последний
            leaves = [
...
class Sign:
    """HMAC подписи, RSA-PSS, ECDSA, Ed25519."""
    ...
    def verify(
        ...
        """Безопасная проверка HMAC (защита от timing-атак)."""
    ...
    def poly1305(self, key: KeyLike, data: DataLike) -> str:
        """Poly1305 MAC (32-байт тег). Требует уникального ключа на каждое сообщение!"""
        _require(CRYPTO_AVAILABLE, "cryptography")
...
class Utils:
    """Утилиты: случайные данные, анализ, разделение ключей, OTP, TOTP."""
    ...
    def random_int(self, low: int, high: int) -> int:
        if low > high:
            raise ValueError("low должно быть ≤ high")
    ...
    def zeroize(self, data: Union[bytearray, memoryview]) -> None:
        """
        Безопасно затирает содержимое bytearray нулями.
        Уменьшает время нахождения секретов в памяти.
        Примечание: не гарантирует затирание на уровне ОС (Python GC).
        """
    ...
    def timing_safe_sleep(self, base_ms: float = 100.0, jitter_ms: float = 50.0) -> None:
        """
        Случайная задержка для предотвращения timing-атак в сетевых протоколах.
        Полезно при проверке паролей/токенов в API-обработчиках.
        """
        delay = (base_ms + secrets.randbelow(int(jitter_ms * 1000)) / 1000.0) / 1000.0
        time.sleep(delay)

    # ── Разделение ключа (XOR-схема) ──────────────────────────────────────────

    def split_key(self, key: str, n: int = 3) -> List[str]:
        """
        Разделение ключа на n частей (XOR-схема).
        Для восстановления нужны все n частей.
        """
        if n < 2:
    ...
    def recover_key(self, shares: List[str]) -> str:
        """Восстановление ключа из всех частей."""
        if len(shares) < 2:
            raise ValueError("Нужно минимум 2 части")
        parts  = [bytes.fromhex(s) for s in shares]
        if len(set(len(p) for p in parts)) > 1:
            raise ValueError("Все части должны быть одного размера")
        result = parts[0]
        for p in parts[1:]:
    ...
    def otp_encrypt(self, data: DataLike) -> Tuple[str, str]:
        """
        One-Time Pad шифрование.
        Информационно-теоретически стойко при корректном использовании.
    ...
    def totp(self, secret: str, digits: int = 6, period: int = 30) -> str:
        """TOTP (RFC 6238) — совместим с Google Authenticator, Authy и т.д."""
    ...
    def is_encrypted(self, data: str) -> bool:
        """Эвристика: высокая энтропия base64 → похоже на шифртекст."""
        try:
            raw = _b64dec(data)
    ...
    def diff(self, a: DataLike, b: DataLike) -> Dict[str, Any]:
        """Сравнение двух данных: различия и статистика."""
    ...
    def benchmark(self, key: Optional[str] = None, size: int = 1024) -> Dict[str, Any]:
        """Замер скорости всех доступных алгоритмов."""
...
class CryptoVault:
    """
    Зашифрованное хранилище секретов на основе AES-GCM (с fallback на WordLock).

    Хранит пары ключ-значение в зашифрованном JSON-файле.
    Поддерживает контекстный менеджер и автосохранение.
    ...
    def _decrypt_vault(self, data: bytes) -> bytes:
        ...
        if CRYPTO_AVAILABLE:
            try:
                ...
                raise DecryptionError("Неверный мастер-пароль или хранилище повреждено")
        else:
            mac_given = ct[-16:]
            ct_body   = ct[:-16]
            mac_calc  = _hmac.new(k, ct_body, "sha256").digest()[:16]
            if not _hmac.compare_digest(mac_calc, mac_given):
                raise IntegrityError("Хранилище повреждено или неверный пароль")
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            raise VaultError(f"Повреждённый файл хранилища: {e}") from e

    def _save(self) -> None:
        _ = self._derive_master_key()  # гарантируем наличие salt
    ...
    def get_meta(self, name: str) -> Optional[Dict[str, Any]]:
        """Получает полную запись секрета с метаданными."""
        return self._data.get(name)

    def delete(self, name: str) -> bool:
        """Удаляет секрет. Возвращает True если был найден."""
    ...
    def list_keys(self, tag: Optional[str] = None) -> List[str]:
        """Список ключей, опционально фильтрованный по тегу."""
        if tag is None:
            return list(self._data.keys())
        return [k for k, v in self._data.items() if tag in v.get("tags", [])]

    def rename(self, old_name: str, new_name: str) -> None:
        """Переименовывает секрет."""
        if old_name not in self._data:
            raise VaultError(f"Секрет не найден: {old_name}")
        if new_name in self._data:
            raise VaultError(f"Имя уже занято: {new_name}")
    ...
    def export_keys(self, path: str, password: str) -> None:
        """Экспортирует хранилище с новым паролем."""
    ...
    def change_password(self, new_password: str) -> None:
        ...
        self._salt   = None  # пересоздаём salt
        self._save()
...
# Base58 — исправленная реализация
...
def batch_encrypt(
    ...
    """
    Пакетное шифрование списка строк/байтов одним ключом.

    Args:
        items: Список данных для шифрования.
        key:   Ключ (если None — генерируется).
        algo:  Алгоритм шифрования (aes_gcm | aes256 | chacha20_poly1305 | wordlock).
    Returns:
        (encrypted_list, key)
...
def batch_decrypt(
    ...
    Если один элемент не расшифровывается — выбрасывает DecryptionError с индексом.
    """
    dec_fn  = getattr(decrypt, algo)
    results = []
    for i, item in enumerate(items):
        try:
            ...
            raise DecryptionError(f"Ошибка при дешифровании элемента #{i}: {e}") from e
    return results
...
def encrypt_file(
    ...
    """
    Шифрует файл AES-GCM (весь файл загружается в память).
    Для больших файлов используйте encrypt_stream.
...
def decrypt_file(
    ...
    """Дешифрует файл, зашифрованный encrypt_file."""
    _require(CRYPTO_AVAILABLE, "cryptography")
    src = Path(path)
...
def encrypt_stream(
    ...
    """
    Потоковое шифрование файла AES-GCM (поддерживает файлы любого размера).

    Каждый чанк шифруется отдельно с уникальным nonce.
    Формат файла:
      [6 байт магия][32 байт соль ключа][4 байт # чанков][чанки...]
      Чанк: [4 байт длина CT][12 байт nonce][CT]

    Args:
        src_path:   Путь к исходному файлу.
        dst_path:   Путь к выходному файлу.
        key:        Ключ (если None — генерируется).
        chunk_size: Размер чанка в байтах (по умолчанию 64 КБ).
    Returns:
        Использованный ключ.
    """
    _require(CRYPTO_AVAILABLE, "cryptography")
    ...
    # Собираем все чанки сначала чтобы знать их количество
    chunks: List[bytes] = []
...
def decrypt_stream(
    ...
    """
    Потоковое дешифрование файла, зашифрованного encrypt_stream.
    Returns: dst_path
    """
    _require(CRYPTO_AVAILABLE, "cryptography")
    ...
    with open(src, "rb") as f:
        ...
        if magic != _STREAM_MAGIC:
            raise DecryptionError(
                f"Неверный формат файла (магия: {magic.hex()}, ожидалось: {_STREAM_MAGIC.hex()})"
            )
        _salt      = f.read(32)
        ...
        with open(dst, "wb") as out:
            for chunk_idx in range(num_chunks):
                ...
                try:
                    ...
                    raise DecryptionError(
                        f"Ошибка дешифрования чанка #{chunk_idx} — неверный ключ или файл повреждён"
                    )
                out.write(plaintext)
...
def capabilities() -> Dict[str, Any]:
    """Возвращает список доступных возможностей и версии зависимостей."""
    return {
        "version":       __version__,
        "dependencies": {
...
def demo():
    ...
    print(f"\n{SEP}")
    print(f"  cryptolibo v{__version__} — демонстрация")
    print(f"  cryptography: {CRYPTO_VERSION or 'н/д'}  |  "
          f"pycryptodome: {PYCRYPTODOME_VERSION or 'н/д'}  |  "
          f"argon2: {ARGON2_VERSION or 'н/д'}")
    print(SEP)

    # 1. Ключи
    print("\n[1] Генерация ключей")
    key = generate_key(256)
    print(f"  256-bit hex ключ : {key[:32]}...")
    b64_key = generate_key(256, encoding="base64")
    print(f"  256-bit b64 ключ : {b64_key[:32]}...")
    pw_key, pw_salt = keys.password_to_key("my_password", algo="pbkdf2")
    print(f"  PBKDF2 ключ      : {pw_key[:32]}...")
    ki = keys.info(key)
    print(f"  Качество ключа   : {ki['quality']} ({ki['entropy_per_byte']} бит/байт)")

    # 2. Симметричное шифрование
    secret = "Привет, мир! Hello 🔐"
    print(f"\n[2] Симметричное шифрование  (данные: '{secret}')")
    if CRYPTO_AVAILABLE:
        _test("AES-GCM (рекомендуется)",  encrypt.aes_gcm,           decrypt.aes_gcm,           key, secret)
    ...
    # 3. WordLock
    print(f"\n[3] WordLock v2")
    for word in ["sigma", "correct horse battery staple", "Ключ123"]:
    ...
    try:
        ...
        print(f"  Неверное слово: должна быть ошибка {FAIL}")
    except (DecryptionError, IntegrityError):
        print(f"  Неверное слово → IntegrityError {PASS}")
    ...
    # 7. Пакетное шифрование
    print("\n[7] Batch encrypt / decrypt")
    items = ["секрет1", "secret_two", "третий 🔑"]
    enc_list, bkey = batch_encrypt(items)
    dec_list = batch_decrypt(enc_list, bkey)
    ok = dec_list == items
    print(f"  batch_encrypt  : {PASS if ok else FAIL}  {len(items)} элементов")
    ...
    try:
        ...
        print(f"  vault size     : {len(v2)} секретов")
        try:
            CryptoVault(vault_path, "wrong_password")
            print(f"  Неверный пароль: {FAIL}")
        except (DecryptionError, IntegrityError):
            print(f"  Неверный пароль → ошибка {PASS}")
    finally:
        try:
    ...
    # 9. Потоковое шифрование файлов
    print("\n[9] Потоковое шифрование файлов (encrypt_stream)")
    if CRYPTO_AVAILABLE:
        ...
        try:
            ...
            print(f"  200 KB файл    : {PASS if ok else FAIL}")
        finally:
    ...
    print(f"  Энтропия ключа   : {utils.entropy(bytes.fromhex(key)):.2f} бит/байт")
    ...
    for algo, info in sorted(bench.items(), key=lambda x: x[1].get("ms", 9999)):
        if "error" in info:
            print(f"  {algo:<28}: ошибка — {info['error']}")
        else:
            print(f"  {algo:<28}: {info['ms']:>7} мс  ({info['throughput_mb_s']:>6} MB/s)")
